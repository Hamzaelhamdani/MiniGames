...
Page({
  data: {
    ...
    waste: [],
    // Défausse (cartes retournées)
    foundations: [[], [], [], []],
    // 4 piles de fondation (As → Roi)
    tableau: [[], [], [], [], [], [], []],
  ...
  onLoad() {
    const sysInfo = ma.getSystemInfoSync();
    this.setData({
  ...
  canMoveToFoundation(card, foundationIndex) {
    ...
    if (foundation.length === 0) {
      // Fondation vide: seul un As peut être placé
      return card.value === 'A';
    }
  ...
  moveToFoundation(card, cards, source, foundationIndex) {
    ...
    foundations[foundationIndex].push({
      ...card
    });
  ...
  onWasteTap() {
    ...
    if (foundationIdx >= 0) {
      this.moveToFoundation(topCard, [topCard], {
        type: 'waste'
      }, foundationIdx);
      return;
    }
  ...
  onTableauCardTap(e) {
    ...
    if (this.data.selectedCards.length > 0 && this.data.selectedSource) {
      ...
      if (card.id === targetCard.id) {
        ...
        const success = this.moveToTableau(this.data.selectedCards, this.data.selectedSource, colIdx);
        if (!success) {
          this.clearSelection();
        }
        return;
    ...
    // Carte du dessus: essayer auto-move vers fondation
    if (cardIdx === column.length - 1) {
      const foundationIdx = this.findFoundationForCard(card);
      if (foundationIdx >= 0) {
        this.moveToFoundation(card, [card], {
          type: 'tableau',
          col: colIdx,
          cardIndex: cardIdx
        }, foundationIdx);
  ...
  onFoundationTap(e) {
    ...
    const success = this.moveToFoundation(this.data.selectedCards[0], this.data.selectedCards, this.data.selectedSource, foundationIdx);
    if (!success) {
      this.clearSelection();
      ma.vibrateShort({
        type: 'heavy'
  ...
  onDragEnd(e) {
    ...
    if (dropY < this.data.topRowHeight + 80 && this.data.selectedCards.length === 1) {
      ...
      if (dropX > foundationStartX) {
        ...
        if (foundationIdx >= 0 && foundationIdx < 4) {
          moved = this.moveToFoundation(this.data.selectedCards[0], this.data.selectedCards, this.data.selectedSource, foundationIdx);
        }
      }
    }
    ...
    if (!moved && dropY >= this.data.topRowHeight - 50) {
      ...
      if (bestCol >= 0) {
        moved = this.moveToTableau(this.data.selectedCards, this.data.selectedSource, bestCol);
      }
    }
    if (!moved) {
      ma.vibrateShort({
