...
class FirebaseAnalytics {
  ...
  storeEvent(event) {
    try {
      let events = wx.getStorageSync('firebase_events') || [];
      events.push(event);
      ...
      wx.setStorageSync('firebase_events', events);
    } catch (e) {
  ...
  getClientId() {
    let clientId = wx.getStorageSync('firebase_client_id');
    if (!clientId) {
      clientId = 'user_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
      wx.setStorageSync('firebase_client_id', clientId);
    }
    return clientId;
  }
  ...
  getStoredEvents() {
    return wx.getStorageSync('firebase_events') || [];
  }

  // Nettoyer les événements stockés
  clearStoredEvents() {
    wx.setStorageSync('firebase_events', []);
  }
}
