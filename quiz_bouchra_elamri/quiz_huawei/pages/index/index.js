...
const QUESTIONS = [{
  ...
  q: 'What is the boiling point of water at sea level in °C?',
  ...
  q: 'Which country hosts the city of Tokyo?',
  ...
  q: 'Which country is the origin of the sport karate?',
  ...
  options: ['Mercury', 'Gold', 'Copper', 'Aluminium'],
  a: 0
}, {
  q: 'Which month has 28 days every year?',
  options: ['February', 'February only in leap years', 'February and others', 'None'],
  a: 0
}, {
  ...
  options: ['Nikola Tesla', 'Albert Einstein', 'Galileo Galilei', 'Marie Curie'],
  a: 1
}, {
  ...
  q: 'Which animal is known for changing colors to blend in?',
  options: ['Chameleon', 'Cheetah', 'Kangaroo', 'Panda'],
  a: 0
}, {
  q: 'Which organ is primarily responsible for filtering blood?',
  ...
  q: 'Which famous physicist developed laws of motion?',
...
Page({
  data: {
    ...
    // 'start' | 'quiz' | 'result'
    questions: [],
    // selected 20 questions for session
    currentIndex: 0,
    score: 0,
  ...
  startQuiz() {
    ...
    this._locked = false;
    console.log('[quiz] startQuiz chosen questions:', selected.length);
    this.setData({
      screen: 'quiz',
      questions: selected,
      currentIndex: 0,
  ...
  chooseOption(e) {
    ...
    this._locked = true;
    // fallback: ensure lock is cleared after 1.5s in case rendering or setData callback fails
    if (this._lockTimeout) clearTimeout(this._lockTimeout);
    this._lockTimeout = setTimeout(() => {
      this._locked = false;
      this._lockTimeout = null;
    }, 1500);
    console.log('[quiz] chooseOption start', { currentIndex: this.data.currentIndex, locked: this._locked });
    try {
      ...
      if (!question) {
        console.error('[quiz] chooseOption: missing question at index', currentIndex, 'questions length=', questions.length);
        // recover by showing result screen to avoid stuck state
      ...
      if (nextIndex >= questions.length) {
        // finish: set all fields including screen in one call to avoid nested setData pitfalls
        console.log('[quiz] finishing quiz', { currentIndex, nextIndex, questionsLength: questions.length, scoreBefore: score });
        ...
        this.setData({
          ...
          try {
            ...
            console.error('[quiz] error clearing resultAnim', err);
          }
          if (this._lockTimeout) {
            clearTimeout(this._lockTimeout);
            this._lockTimeout = null;
        ...
        this.setData({ score, currentIndex: nextIndex }, () => {
          console.log('[quiz] moved to next question', { nextIndex });
          if (this._lockTimeout) {
            clearTimeout(this._lockTimeout);
            this._lockTimeout = null;
          }
      ...
      console.error('[quiz] chooseOption unexpected error', err);
      if (this._lockTimeout) {
        clearTimeout(this._lockTimeout);
        this._lockTimeout = null;
      }
      ...
      try {
        ...
        console.error('[quiz] failed to set result screen in recovery', e);
      }
    }
  },
  restartQuiz() {
    // clear any pending lock timeout when restarting
    if (this._lockTimeout) {
      clearTimeout(this._lockTimeout);
      this._lockTimeout = null;
    }
    console.log('[quiz] restartQuiz');
    this.setData({
      resultAnim: '',
      isGood: false,
      resultMessage: ''
